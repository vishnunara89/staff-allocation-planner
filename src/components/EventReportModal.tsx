            ...
            <div className="modal-container" onClick={e => e.stopPropagation()} style={{ maxWidth: '650px' }}>
                <div className="modal-header">
                    <div className="modal-header-title">
                        <span style={{ color: '#7C4C2C', fontSize: '0.8rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em' }}>Event Analysis</span>
                        <h3 style={{ marginTop: '0.2rem' }}>Staffing Report</h3>
                    </div>
                    ...
                    {loading ? (
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
                            ...
                            <div style={{
                                ...
                                <div>
                                    <div style={{ fontSize: '0.85rem', opacity: 0.8 }}>Venue</div>
                                    <div style={{ fontSize: '1.25rem', fontWeight: 600 }}>{venue?.name}</div>
                                </div>
                                <div style={{ textAlign: 'right' }}>
                                    <div style={{ fontSize: '0.85rem', opacity: 0.8 }}>Expected Guests</div>
                                    <div style={{ fontSize: '1.5rem', fontWeight: 700 }}>{event.guest_count}</div>
                                </div>
                                <div style={{ width: '1px', height: '40px', background: 'rgba(255,255,255,0.2)' }}></div>
                                <div style={{ textAlign: 'right' }}>
                                    <div style={{ fontSize: '0.85rem', opacity: 0.8 }}>Total Staff Needed</div>
                                    <div style={{ fontSize: '1.5rem', fontWeight: 700 }}>{totalNeeded}</div>
                                </div>
                            </div>

                            {/* Requirements Table */}
                            <div>
                                <h4 style={{ fontSize: '1rem', color: '#1a1a1a', marginBottom: '1rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                                    <span style={{ fontSize: '1.2rem' }}>📋</span> Baseline Requirements
                                </h4>
                                <div style={{ background: '#f8fafc', borderRadius: '16px', overflow: 'hidden', border: '1px solid #e2e8f0' }}>
                                    <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                                        <thead style={{ background: 'rgba(124, 76, 44, 0.05)' }}>
                                            <tr>
                                                <th style={{ textAlign: 'left', padding: '1rem', fontSize: '0.8rem', color: '#64748b', textTransform: 'uppercase' }}>Role</th>
                                                <th style={{ textAlign: 'center', padding: '1rem', fontSize: '0.8rem', color: '#64748b', textTransform: 'uppercase' }}>Count</th>
                                                <th style={{ textAlign: 'left', padding: '1rem', fontSize: '0.8rem', color: '#64748b', textTransform: 'uppercase' }}>Calculation Rule</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            {requirements.map((req, i) => (
                                                <tr key={i} style={{ borderTop: '1px solid #e2e8f0' }}>
                                                    <td style={{ padding: '1rem', fontWeight: 600, color: '#1e293b' }}>{req.roleName}</td>
                                                    <td style={{ padding: '1rem', textAlign: 'center' }}>
                                                        <span style={{ background: '#EFE8E3', color: '#7C4C2C', padding: '0.3rem 0.7rem', borderRadius: '8px', fontWeight: 700 }}>{req.count}</span>
                                                    </td>
                            ...
                            {specialReqs.length > 0 && (
                                <div>
                                    <h4 style={{ fontSize: '1rem', color: '#1a1a1a', marginBottom: '1rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                                        <span style={{ fontSize: '1.2rem' }}>✨</span> Additional Special Requirements
                                    </h4>
                ...
                <div className="modal-footer" style={{ borderTop: '1px solid #f1f5f9' }}>
                    <button type="button" className={styles.buttonCancel} onClick={onClose}>Close Report</button>
                    <button
                        className={styles.buttonSubmit}
                        onClick={() => {
                            onClose();
                            // Logic to open generation or just close
                        }}
                    >
                        Generate All Plans
                    </button>
